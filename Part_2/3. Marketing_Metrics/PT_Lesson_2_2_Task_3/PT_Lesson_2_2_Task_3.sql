...
campaign AS 
    ...
    WHERE user_id IN (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741, 
        ...
        AND user_id IN (SELECT user_id
                        FROM not_canceled_orders)
        AND date >= '2022-09-01' AND date < '2022-09-08'
    ),
avg_check_per_user AS
    (
    SELECT campaign.user_id, 
           campaign.ads_campaign,
           ROUND(SUM(products.price) ::numeric / COUNT(DISTINCT campaign.order_id),2) as avg_user_check
    FROM campaign
    LEFT JOIN products
    USING(product_id)
    GROUP BY campaign.user_id, campaign.ads_campaign
    )
SELECT ads_campaign,
       ROUND(SUM(avg_user_check) ::numeric / COUNT(user_id), 2) AS avg_check
FROM avg_check_per_user
