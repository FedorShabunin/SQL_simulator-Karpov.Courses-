...
first_action AS
    ...
    SELECT ads_campaign,
           user_id,
           MIN(DATE_TRUNC('month', date) ::DATE) OVER (PARTITION BY user_id) AS start_month,
...
SELECT ads_campaign,
       ...
       ROUND(COUNT(DISTINCT user_id) ::numeric / MAX(COUNT(DISTINCT user_id)) OVER (PARTITION BY start_date, ads_campaign), 2) AS retention
FROM first_action
GROUP BY ads_campaign, date, start_month, start_date
HAVING date - start_date IN (0, 1, 7) AND start_date = '2022-09-01'
ORDER BY ads_campaign, start_date, date, day_number;
